...
for (( X=0 ; $X < $MAX_CYCLES ; X+=1 )); do
    ...
    skmanage update --type sslProfile --name connector-ssl-profile caCertFile=$CERTPATH/ca-certificate.pem certFile=$CERTPATH/client-certificate.pem privateKeyFile=$CERTPATH/client-private-key.pem password=client-password 
    # CLIENT_PIDS+="$! "
