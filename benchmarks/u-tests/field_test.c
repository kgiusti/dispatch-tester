...
static char *check_advance(void *context, qd_iterator_t *iter,
                           const char *addr, const char *view)
{
    char *ret = 0;
    qd_iterator_reset(iter);
    const int len = qd_iterator_length(iter);
    qd_iterator_advance(iter, len);
    const int rem = qd_iterator_remaining(iter);
    if (rem) {
        snprintf(fail_text, FAIL_TEXT_SIZE, "Addr '%s' advance failed.  advanced %d remaining %d\n",
                 addr, len, rem);
        ret = fail_text;
    }
    return ret;
                             ...
                             const char *addr, const char *view)
{
    ...
    if (!ret)
        ret = check_advance(context, iter, addr, view);
    return ret;
}
